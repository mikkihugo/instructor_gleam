...
pub fn main() {
  ...
  let sentiment_model =
    instructor.string_response_model(
  ...
  let messages = [
    instructor.system_message(
      "You are a sentiment analysis expert. Always respond with exactly one word: positive, negative, or neutral.",
    ),
    instructor.user_message(
      "I absolutely love programming in Gleam! It's such a wonderful language.",
    ),
  ]
  ...
  case
    instructor.chat_completion(
      ...
      Some(10),
      // max_tokens (just need one word)
      Some(types.Json),
      // mode
      None,
      // max_retries (use default)
      None,
      // validation_context
    )
  {
...
pub fn extract_person_info() {
  ...
  let messages = [
    instructor.system_message(
      "Extract person information from the text and return as JSON.",
    ),
    instructor.user_message(
...
pub fn ollama_example() {
  ...
  let messages = [
    instructor.user_message(
      "Gleam is a friendly language for building type-safe systems that scale! It compiles to Erlang and JavaScript.",
    ),
  ]
  ...
  case
    instructor.chat_completion(
      ...
      Some(50),
      Some(types.MdJson),
      // Ollama works well with markdown JSON
      None,
      None,
    )
  {
