...
pub fn sse_done_event_test() {
  let done_event =
    sse_parser.SSEEvent(event_type: None, data: "[DONE]", id: None, retry: None)
...
pub fn json_parser_simple_object_test() {
  let state = json_parser.init_parser()
  let #(_final_state, completed) =
    json_parser.parse_chunk(state, "{\"key\": \"value\"}")
...
pub fn json_parser_nested_object_test() {
  let state = json_parser.init_parser()
  let #(_final_state, completed) =
    json_parser.parse_chunk(state, "{\"outer\": {\"inner\": \"value\"}}")
...
pub fn json_parser_multiple_objects_test() {
  let state = json_parser.init_parser()
  let #(_final_state, completed) =
    json_parser.parse_chunk(state, "{\"first\": 1}{\"second\": 2}")
...
pub fn json_parser_string_with_braces_test() {
  let state = json_parser.init_parser()
  let #(_final_state, completed) =
    json_parser.parse_chunk(state, "{\"message\": \"Hello {world}\"}")
