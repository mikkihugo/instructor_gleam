...
pub fn custom_validator_decode_error_test() {
  let val = validator.custom_validator(decode.string, fn(s) { Ok(s) })
...
pub fn enum_validator_valid_test() {
  let val =
    validator.enum_validator(["active", "inactive", "pending"], "Status")
...
pub fn list_validator_valid_test() {
  let val =
    validator.list_validator(
      decode.string,
      validator.string_length_validator(2, 10, "Tag"),
    )
